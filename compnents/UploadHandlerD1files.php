...
class UploadHandlerD1files extends UploadHandler {
    ...
    static function getUploadDirPath($model_name){
        $upload_dir_parh = realpath(Yii::getPathOfAlias(Yii::app()->getModule('d1files')->upload_dir));
    ...
    static function createSaveFileName($d1files_id, $file_name) {
        $a = explode('.',$file_name);
        return 'd1_' . $d1files_id . '.' . array_pop($a);
    }
    ...
    public function saveToDb($file_name,$model_name,$model_id) {
        $model = new D1files;
        $model->type = D1files::TYPE_DOCUMENT;
